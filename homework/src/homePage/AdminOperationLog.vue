<template>
  ...
  <div class="c-table">
    <el-table
      ...
      <el-table-column prop="num" label="序号" width="100px"></el-table-column>
      <el-table-column prop="id" label="操作者id" width="100px"></el-table-column>
      <el-table-column prop="operationTime" label="操作时间" width="200px" sortable></el-table-column>
      <el-table-column prop="operationType" label="操作类型" width="150px"></el-table-column>
...
let allData=ref([])
function getAllData() {
  axios.get("http://localhost:8080/getAdminLog")
  .then(response=>{
...
function handleSearch(){
  if (input1.value.length>0){
    ...
    axios.get("http://localhost:8080/getOneAdminLog" + parseInt(input1.value))
        .then(response=>{
          allData.value=response.data
          total.value=allData.value.length//获得总数据条数
          fetchPageData();
